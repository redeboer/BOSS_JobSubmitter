...
function DeleteAllEmptyLines() { # Delete all empty lines in a file, including files that only contain whitespace characters
...
function FormatTextFileToCppVectorArguments() { # Feed this function a text file, and it will prepend a `\t"` and append a `",` to each line. The comma is ommited for the last line.
  # * Import function arguments
  local fileName=${1}
...
function SplitTextFile() { # Feed this function a path to a text file ($1) and it will split up this file into separate files each with a maximum number of lines ($2 -- default value is 10).
  # * Import function arguments
  local fileToSplit="${1}"
  ...
  for file in $(#! number of ? should match the -a3 argument above
    ls ${prefix}???
  ); do
...
function CreateFilenameInventoryFromFile() { # Feed this function a path ($1) to a file containing directories and/or file names and it will list all files within those directories including their absolute paths.  This list will be written to text files ($2) with a maximum number of paths per file ($3 -- default is 0, namely no max). If you wish, you can only list files of a certain extension ($4).
  # * Import function arguments * #
  local inputFile="${1}"
  local outputFile="${2}"
  local maxNLines=${3:-0}
